...
class Buchi(object):
    ...
    def get_subgraph(self, init, accept, is_nonempty_self_loop, next_vertex, part='prefix'):
        """
        ...
        # without self loop, for suffix part, creat two vertices
        # for prefix part, do nothing
        elif part == 'suffix':
            subgraph.add_node('artificial', label=self.buchi_graph.nodes[next_vertex]['label'],
                              neg_label=self.buchi_graph.nodes[next_vertex]['neg_label'],
                              formula=self.buchi_graph.nodes[next_vertex]['formula'])
            subgraph.add_edge(accept, 'artificial', label='1',
                              neg_label=[], formula=to_dnf('1'))

            subgraph.add_node('next_artificial', label='1',
                              neg_label=[], formula=to_dnf('1'))
            subgraph.add_edge('artificial', 'next_artificial', label='1',
                              neg_label=[], formula=to_dnf('1'))
            for path in paths:
                path.append('artificial')
                path.append('next_artificial')
